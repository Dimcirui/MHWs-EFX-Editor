...
`52248353b07b97d8e67493f5ac3ce67ebc01e390` 升级，见 PLAN.md）。以下清单是升级前
（`5224835`）针对 `MHWILDS_EXTRACT/EFX` 官方语料的诊断记录，**升级后尚未重新跑一遍这 9241
个文件复核**（这次升级的验证范围只覆盖了本仓库自带的 2 个样本 + JSON 序列化层，不是这份
清单针对的二进制解析层问题）——下次能访问那份语料时应该整批重新过一遍，确认哪些已经被
`ebb1bc7` 顺带解决。
